...
def _safe_stage(logger: logging.Logger, label: str, records: Iterable[Dict[str, Any]], function) -> List[Dict[str, Any]]:
    output: List[Dict[str, Any]] = []
    for record in records:
        try:
            record.update(function(record) or {})
        except Exception as exc:
            logger.error("%s failed for %s: %s", label, record.get("business_name", "unknown"), exc)
            record.setdefault("pipeline_errors", []).append(f"{label}: {exc}")
        output.append(record)
    return output


def _stage(logger: logging.Logger, label: str) -> None:
    logger.info("[%s]", label)
...
def run_campaign(config: MuseConfig) -> Dict[str, Any]:
    ...
    _stage(logger, "CLEANING")
    cleaned = [normalize_business_record(record) for record in raw[:max(0, config.limit)]]
    checkpoint.save_json("02_cleaned", cleaned)

    _stage(logger, "DEDUPLICATION")
    deduped, rejected = deduplicate_businesses(cleaned)
    logger.info("Retained %d records; rejected %d duplicate or malformed records", len(deduped), len(rejected))
    checkpoint.save_json("02_cleaned", deduped)

    _stage(logger, "VERIFICATION")
    ...
    _stage(logger, "SCORING")
    for item in classified:
        item.update(calculate_lead_score(item))
        item["outreach_ready"] = "YES" if item.get("lead_score", 0) >= config.min_lead_score and item.get("lead_category") not in {"NOT_A_TARGET", "WEBSITE_STATUS_UNCERTAIN"} else "NO"
    ...
    write_run_report(output / "run_report.txt", {"start_time": started.isoformat(timespec="seconds"), "end_time": datetime.now().isoformat(timespec="seconds"), "country": config.country, "region": config.region, "city": config.city, "niche": config.niche, "mode": config.mode, "requested_leads": config.limit, "discovered": len(raw), "cleaned": len(cleaned), "deduplicated": len(deduped), "verified": len(verified), "classified": len(classified), "final_leads": len(final), "rejected": len(rejected_rows), "source_failures": source_failures})
...
def main() -> None:
    ...
    if config.resume:
        print("[RESUME] Checkpoint-aware run requested", flush=True)
    run_campaign(config)
